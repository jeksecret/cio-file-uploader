...
export default function FacilityHome() {
  ...
  const handleDocumentUpload = async (requiredDocId, files) => {
    setUploadingKey(`doc-${requiredDocId}`);
    setError("");
    try {
      const formData = new FormData();
      files.forEach((f) => formData.append("files", f));
      await fetchWithAuth(`/api/uploads/${requiredDocId}`, { method: "POST", body: formData }, getAccessToken);
      loadFacility();
    } catch (err) {
  ...
  const handleOtherUpload = async (files) => {
    setUploadingKey("other");
    setError("");
    try {
      const formData = new FormData();
      files.forEach((f) => formData.append("files", f));
      await fetchWithAuth("/api/uploads/other", { method: "POST", body: formData }, getAccessToken);
      loadFacility();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  if (!facility) {
    return <div className="min-h-screen bg-gray-100 grid place-items-center text-gray-500">読み込み中…</div>;
  }
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-md shadow p-6">
          ...
          <dl className="text-sm space-y-2">
            <div className="flex justify-between"><dt className="text-gray-500">サービス種別</dt><dd className="font-medium text-gray-900">{facility.service_type}</dd></div>
            <div className="flex justify-between"><dt className="text-gray-500">提出期限</dt><dd className="font-medium text-gray-900">{facility.submission_deadline ?? "-"}</dd></div>
          </dl>
        </div>
        ...
        <div className="bg-white rounded-md shadow p-6">
          ...
          <ul className="divide-y divide-gray-100">
            {facility.documents.map((doc) => (
              <li key={doc.required_doc_id} className="py-3">
